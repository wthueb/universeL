...
{
	// assumes:
	// - GetWindowsDirectoryA will not fail
	// - the registry key exists
	// - the subkeys are ordered alphabetically
	// - the name and data of the subkeys are no longer than MAX_PATH
	...
	uint8_t keyData[MAX_PATH];
	std::wstring wsFontFile;

	while (true)
	{
		uint32_t keyNameSize = MAX_PATH;
		uint32_t keyDataSize = MAX_PATH;
		uint32_t keyType;
		...
		auto error = RegEnumValueA(
			...
			keyIndex,
			keyName,
			reinterpret_cast<DWORD*>(&keyNameSize),
			0,
			reinterpret_cast<DWORD*>(&keyType),
			keyData,
			reinterpret_cast<DWORD*>(&keyDataSize));
		...
		if (error || keyType != REG_SZ)
		{
			continue;
		}

		if (_strnicmp(name.data(), keyName, name.size()) == 0)
		{
			path = fontsFolder + std::string(reinterpret_cast<char*>(keyData), keyDataSize);
			RegCloseKey(registryKey);
			return true;
		}
	}
...
void DrawManager::AddCircle(const ImVec2 &center, float radius, ImU32 col, int num_segments /*= 12*/, float thickness /*= 1.0f*/)
{
	_drawList->AddCircle(center, radius, col, num_segments, thickness);
}

void DrawManager::AddCircleFilled(const ImVec2 &center, float radius, ImU32 col, int num_segments /*= 12*/)
{
	_drawList->AddCircleFilled(center, radius, col, num_segments);
}
